...
from bigquery.google_cloud.big_query import BigQuery, QueryError
...
def run_fetch_transaction():
    ...
    service_account_files = glob.glob(os.path.join('/opt/airflow/dags/bigquery/service_account', "*.json"))
    ...
    assert os.path.exists(f'/opt/airflow/dags/bigquery/temp_tx_{date.today() - timedelta(days=1)}.csv')
